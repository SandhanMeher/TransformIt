...
export default function PdfCompression() {
    ...
    const handleFileChange = (e) => {
        ...
        if (selectedFile && selectedFile.type === 'application/pdf') {
            ...
            setFileInfo(`Original file size: ${(selectedFile.size / 1024 / 1024).toFixed(2)} MB`);
        } else {
        ...
        try {
            ...
            for (const page of pages) {
                ...
                for (const img of images) {
                    ...
                    page.drawImage(scaledImage, { x: img.x, y: img.y, width: img.width, height: img.height });
                }
            }

            const optimizedPdf = await pdfDoc.save();
            const blobUrl = URL.createObjectURL(new Blob([optimizedPdf], { type: 'application/pdf' }));
    ...
    return (
        <div className='h-screen flex items-center justify-center bg-gradient-to-r from-[#092C2B] to-[#1E685F]'>
            <div className='w-[90%] max-w-2xl bg-white rounded-lg p-8 shadow-lg'>
                <h1 className='text-2xl font-bold text-center mb-6 text-gray-800'>PDF Compression Tool</h1>
                ...
                {file && (
                    <div className='mb-4'>
                        <label className='block text-gray-700 font-medium mb-2'>Compression Level: {compressionLevel}%</label>
                        <input
                            type='range'
                            min='10'
                            max='90'
